...
An instance of Codey has to be ordered via the Codey website and is running on pre-defined "black-box" OpenShift OKE service clusters to which the customer doesn't have access to.
...
We differentiate between Forgejo by VSHN and Codey, exposing a different XRD depending on the service.
...
See <<_forgejo_by_vshn_2, default Helm values>>.
...
<2> Plans from https://www.codey.ch/#Beginner[Codey Website^]. See also <<Kubernetes Resource Configuration>>.
...
See <<_codey_3, default Helm values>>.
...
Forgejo by VSHN version exposes the configuration parameters to configure the SMTP service.
...
Following are the default Helm values.
...
[source,yaml]
----
include::attachment$example-forgejo-helm-values-codey.yaml[]
----
