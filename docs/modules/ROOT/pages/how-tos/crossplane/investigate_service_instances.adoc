...
This page shows how a service instance can be investigated. It provides several steps and inputs how to analyze issues, it is generic for all services.
...
* `service.syn.tools/name` - Name of the service (i.e. "redis-k8s")
* `service.syn.tools/id` - UID of the service (i.e. Redis = 8d4b8039-6bcc-4f68-98f0-0e8efa5ab0e2)
...
If an instance is not in a ready state and the marketplace shows it as "Installing" or similar, it means that some of the resources that were created for this instance are not ready. To figure out why, go through all referenced resources and see if they provide further info.
...
Since the namespace itself and the PVCs are not created directly by the Helm chart, they are left in place. This enables restoring of a deleted service, by instantiating the service again with the same UID.
