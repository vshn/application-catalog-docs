...
SLA changes can be configured separately and doesn't need any of the setup described below.

We're currently supporting five different plan sizes:
...
[NOTE]
====
It's configurable which plan changes the Service Broker will allow. 
...
So will the Crossplane Composition, but it will fail.

Please make sure to now allow invalid plan changes when configuring the Service Broker.
====
...
It's possible to allow changes to smaller plan sizes, for example `medium>small`, these changes however aren't valid and will fail.
