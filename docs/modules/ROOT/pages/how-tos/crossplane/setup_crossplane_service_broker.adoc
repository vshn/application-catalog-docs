...
- One _Crossplane Service Broker_ knows which services it offers and what already instantiated services its responsible for.
...
[NOTE]
======
Note that the _Service Catalog_ and the _Crossplane Service Broker_ don't necessarily have to run on the same Kubernetes cluster.
A _Service Catalog_ can for example be installed on the Kubernetes cluster of a customer, while the _Crossplane Service Broker_ might be running on a central and dedicated cluster.

Also note that services managed by _Crossplane_ may not necessarily be provisioned on the same cluster on which _Crossplane_ is installed on.
_Crossplane_ does for example support to instantiate cloud resources from providers such as Google Cloud, Azure or AWS.
Such resources naturally don't run on the cluster, on which _Crossplane_ is installed.
======
...
It then updates it's catalog of all the services that can be provided by our (and all other) registered service brokers.
