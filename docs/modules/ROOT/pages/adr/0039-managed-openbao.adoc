...
.Summary
====
This ADR outlines the implementation of a managed OpenBao service on the AppCat platform to provide secret mamanagement capabilities to customers.
...
- Better integration with AppCat framework
...
- Leverage existing AppCat Backup mechanisms (K8sup)
...
3. **Backup Storage**: `VSHNObjectBucket` for Raft snapshots using K8sup
