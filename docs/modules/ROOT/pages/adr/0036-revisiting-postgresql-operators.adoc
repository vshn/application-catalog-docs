...
.Summary
====
StackGres is one of the major pain points we currently struggle with.
CNPG provides a good alternative, even though some features are not quite on par yet.
It has the best UX of the tested operators and EDB wants to partner with us.
====
...
* AWS article about pg_repack: https://docs.aws.amazon.com/prescriptive-guidance/latest/postgresql-maintenance-rds-aurora/pg-repack.html
* Percona: https://www.percona.com/blog/understanding-pg_repack-what-can-go-wrong-and-how-to-avoid-it/
* EDB's alternative: https://www.enterprisedb.com/blog/Release_Radar_Reducing_Bloat_Improving_Query_Performance_EDB_Support_pg_squeeze

Pg_repack is a more thorough `VACUUM` it can clean up the database of old data and free up storage space.
...
Although StackGres still has some advanced features that others are lacking.
Like the automated security maintenance for instance.
...
TimescaleDB can be installed via 3rd party images: https://github.com/clevyr/docker-cloudnativepg-timescale

To use extensions that are not available in the default image, CNPG provides inofficial instructions how to fork and build custom images: https://github.com/cloudnative-pg/postgres-containers/issues/115#issuecomment-2563173289

There are plans to add dynamic extension installations on the roadmap: https://github.com/cloudnative-pg/cloudnative-pg/issues/7188

==== Installation

The community maintained image aren't explicitly tested against OpenShift: https://cloudnative-pg.io/documentation/1.24/supported_releases/#support-status-of-cloudnativepg-releases

However EDB maintains a certified image for OpenShift in the RedHat Image Catalog: https://catalog.redhat.com/software/container-stacks/detail/653fd4035eece8598f66d97b
...
Each sidecar has to be adjusted seperately and then patched for an minorupgrade. The sidecar images also need to match the PostgreSQL version and the operator version as well.
...
The operator supports downloading and installing pre-packaged extensions dynamically: https://docs.percona.com/percona-operator-for-postgresql/2.0/custom-extensions.html?h=extension#adding-custom-extensions
...
Percona supports Databases and User management. Their API is surprisingly close to our own in AppCat: https://docs.percona.com/percona-operator-for-postgresql/2.0/users.html?h=user#custom-users-and-databases
...
** Opens up opportunity to later also provider EDB Operator based PostgreSQL
...
** Additionally it simplifies PostgreSQL version management via their `ImageCatalog`, if we ever decide to provider custom images with specific extensions
* They provide detailed release cycles and EOL dates: https://cloudnative-pg.io/documentation/1.26/supported_releases/#support-status-of-cloudnativepg-releases
