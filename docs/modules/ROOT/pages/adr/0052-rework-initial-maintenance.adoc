...
* Resolve major image tags (e.g. `16`) to the full semver (e.g. `16.4.1`)
...
The pod restarts triggered during the instance bootstrap, can corrupt the state of certain services (eg. MariaDB).
...
. Honour `metadata.appcat.vshn.io/autoUpdate` transitions *immediately*, not at the next maintenance window (this flag switches the instance from prod revision pin to floating latest, i.e. prod->test).
...
Image tag resolution is *not* part of this option — that is handled by the composition functions reading an image catalogue (Option A / Option B).
...
* *Fits the existing model*: reuses composition revisions, `metadata.appcat.vshn.io/revision` label, update policy — the mechanism ADR0030 already established.
...
=== Option H: Mutating admission webhook (SLAP)
...
*Step 1 — Short term (stop the corruption):* adopt *Option E*.
...
*Step 2 — Long term (remove initial maintenance):* adopt *Option A* (or *Option B* — image catalogue as the storage primitive) consumed by the composition functions, plus *Option C* (revision-selector controller) and a dedicated `autoUpdate` controller.

Concretely:

* An image catalogue holds `service + majorVersion -> latest full semver`, refreshed by a weekly updater.
* Composition functions read the catalogue on every reconcile and write the resolved full semver onto the composite (and its managed resources) at render time. New instances render with the correct tag on the first reconcile — no post-bootstrap upgrade.
