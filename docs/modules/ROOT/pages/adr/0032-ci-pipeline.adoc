...
.Summary
====
With the Continuous Delivery (CD) solution nearing completion, we now need to establish a robust Continuous Integration (CI) system to support a complete CI/CD pipeline.
A key requirement is the ability to handle hotfix releases independently of feature deployments.
====
...
However, the current approach is proving problematic for handling hotfix releases under our new CD process.
