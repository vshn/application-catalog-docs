...
Namespace isolation is sufficient on our target platforms, which is also why we prefer the in-cluster approach over CSP VMs.
...
* On *APPUiO Public*, no such sandboxing is needed: customers are already restricted by OpenShift SCCs and can provision arbitrary resources in their own namespaces behind quotas and existing safeguards.
* On *APPUiO Managed*, the cluster belongs to the customer, who can deploy whatever they like on it, so runner isolation is not our concern.
...
* Runner isolation is platform-specific: on Servala job containers run under a gVisor `RuntimeClass`; APPUiO Public relies on existing SCCs and quotas; APPUiO Managed is the customer's own cluster.
