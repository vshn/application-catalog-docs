...
If a dedicated namespace later proves worthwhile (for example for the runner-only AddOn case, or for stricter resource boundaries), we can move it without revisiting the rest of this decision.
...
* Moving the runner to a dedicated namespace later remains possible, which would keep open a future runner-only AddOn where a customer brings their own Forgejo instance.
