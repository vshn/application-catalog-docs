...
Each runner is deployed in its own namespace for isolation.
...
One option is to deploy the AddOn in the Forgejo instance namespace, argued for on the grounds that the runner is tightly coupled to Forgejo and cannot run without it.
We instead place it in a *dedicated namespace*.
Classification as an AddOn is independent of placement, and a separate namespace gives cleaner resource isolation and billing boundaries.
It also keeps the door open to future flexibility, for example a runner-only AddOn where the customer brings their own Forgejo instance.
...
On APPUiO Public, customers already run arbitrary workloads in their namespaces behind quotas and existing safeguards.
On Servala, the Talos-based hardening goes beyond vanilla Kubernetes.
Combined with the runner being confined to its pre-provisioned Pod(s), namespace-level isolation covers the requirement.
...
To keep it simple it deployed into the instance namespace instead of a dedicated one, but it used the same registration mechanism we are deciding on.
