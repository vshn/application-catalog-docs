...
Each Billing CR describes a single service instance and its lifecycle from creation to deletion.
It contains two main sections:

1. **Static data** - Defined under `.spec.odoo`. These values remain unchanged throughout the service's lifecycle.
2. **Dynamic data** - Defined under `.status.events`. This section evolves over time, reflecting changes such as scaling or SLA modifications.

All lifecycle events (creation, scale, deletion, etc.) are captured in the same resource, allowing for complete event history reconstruction.
This enables operations like **resending** events via annotations.

The `.status.events` array must be ordered in **descending** order by `timestamp`, with the most recent event first.

Automatic event resending is supported and includes an **exponential backoff retry mechanism**.

A resource is considered `Synced` only when **all** `.status.events[].state` values are `sent`.

There is currently no need to limit the number of stored events - the expected volume per CR is low and manageable.
...
<3> All Billing CRs reside in the `syn-appcat` namespace.
<4> A finalizer from the controller to protect from accidental deletion.
<5> The field defines after how many days the CR should be deleted.
