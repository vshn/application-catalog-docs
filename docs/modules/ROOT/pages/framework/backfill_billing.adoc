...
The script on this page applies only to the legacy metered billing CronJobs and is only relevant for Object Buckets on Cloudscale/Exoscale.

For VSHN managed services (PostgreSQL, Redis, etc.) that use Event-Based Billing, use the resend annotation instead:
...
See xref:framework/billing.adoc[AppCat Billing] for full documentation.
====
