= VSHN Based AppCat Services (Deprecated)
...
This page describes the legacy Prometheus/Mimir-based metered billing for VSHN managed services.
This system has been superseded by Event-Based Billing using the `BillingService` CRD.

See xref:framework/billing.adoc[AppCat Billing] for up-to-date documentation.

This page is kept for historical reference only.
====

This page explains how VSHN based services handled usage reporting under the legacy metered billing system.
