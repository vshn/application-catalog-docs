...
[NOTE]
====
This page covers metered billing for Object Storage on Cloudscale and Exoscale only.
This system remains active and has **not** been replaced for these services.

For the full billing picture including VSHN managed services (PostgreSQL, Redis, etc.), see xref:framework/billing.adoc[AppCat Billing].
====
