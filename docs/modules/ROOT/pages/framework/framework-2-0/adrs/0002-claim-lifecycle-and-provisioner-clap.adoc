...
* The set of service kinds is open and discovered at runtime. Slapper emits one XR-only XRD per service; Crossplane turns that XRD into the composite CRD, but a v2 XRD no longer carries `claimNames`, so nothing produces the customer-facing claim CRD. The operator must supply that claim CRD and handle arbitrary kinds with no compile-time Go types and no codegen step.
...
=== Claim CRDs generated from the XRDs

Nothing upstream creates the claim CRD: Slapper emits the XRD and Crossplane turns it into the composite CRD, but the claim CRD is missing. So CLAP generates it. A watcher observes the Crossplane XRDs in the `appslap.io` group and, for each one, renders a matching claim CRD.

The claim CRD is the XRD with the composite identity removed: the names have their leading `X` stripped (`XVSHNPostgreSQL` → `VSHNPostgreSQL`), the schema is carried over as-is, and the scope is `Namespaced` so customers create claims in their own namespace. Writes are idempotent, so a schema change on the XRD flows through to the claim CRD.

CLAP does not clean up these CRDs itself. Each claim CRD points back to its XRD as owner, so Kubernetes deletes it automatically when the XRD goes away. XRDs whose kind is not `X`-prefixed, or that are malformed, are skipped.
...
CLAP takes no dependency on Crossplane's code; it treats claims, composites and the XRDs it reads as ordinary Kubernetes objects rather than importing Crossplane's Go types. It does watch the Crossplane XRD type, so that type has to be installed for CLAP to read XRDs, but the operator still builds without a Crossplane dependency and stays decoupled from the in-progress composition and stdlib work.
...
* Any service works with no code generation. A new service XRD is picked up automatically: CLAP derives its claim CRD and starts a controller for it, with no rebuild and no typed API.
* The claim CRD is never hand-authored or kept in sync separately. It is derived from the XRD, so its schema always matches, and it is cleaned up automatically with its XRD via the owner reference.
...
* Treating claims and composites as generic objects gives the operator no schema validation or type safety of its own; correctness rests entirely on the XRDs Slapper emits, the claim CRDs CLAP derives from them, and the composition behind the composite.
* The claim CRD generation depends on the `X`-prefix convention holding: a service whose composite kind is not `X`-prefixed will not get a CLAP-managed claim CRD.
