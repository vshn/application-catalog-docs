...
Controller runtime controllers automatically connect to the Kubernetes API server with four parameters provided by Kubernetes: `KUBERNETES_SERVICE_HOST`, `KUBERNETES_SERVICE_PORT`, `/var/run/secrets/kubernetes.io/serviceaccount/token` and `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt` ({1}[Reference]).
