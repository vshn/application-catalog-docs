...
[source,bash]
----
kubectl --as=system:admin -n $instanceNamespace get pods
----

*Connect to Sentinel*
[source,bash]
----
kubectl --as=system:admin exec -n $instanceNamespace -it $redisPod -- \
  redis-cli \
    -h 127.0.0.1 \
    -p 26379 \
    --tls \
...
[source,bash]
----
kubectl --as=system:admin -n $instanceNamespace rollout restart statefulset redis-node
----
