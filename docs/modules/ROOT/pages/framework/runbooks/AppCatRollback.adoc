...
Use this script to quickly revert affected Claims to a previous CompositionRevision and restore them once the issue is resolved.
...
kubectl --as=system:admin get "\$TYPE" "$NAME" -n "$NAMESPACE" -o jsonpath='{.spec.compositionRevisionSelector.matchLabels}'

echo ""
echo "Watch claim for reconciliation:"
kubectl --as=system:admin get "\$TYPE" "$NAME" -n "$NAMESPACE" -w
----
